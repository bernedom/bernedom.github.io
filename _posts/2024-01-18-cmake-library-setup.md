...
The library will expose a class `Greeter::Hello` that contains a `greet()` function that prints "Hello <name> from a library". This class is declared in the `include/hello/hello.hpp`. Internally it uses a private function called `print_impl` which is defined in The `internal.cpp` and `internal.hpp` files are used to demonstrate how to hide symbols from the library interface. The `GreeterConfig.cmake.in` file is used to configure the CMake package file that will be used to make the library usable with `find_package()`, we will look at this later.
